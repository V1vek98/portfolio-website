      ...
      <Container>
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          {experiences.map((exp, index) => {
            const visualIndex = index >= 5 ? index + 1 : index;
            return (
              <React.Fragment key={exp.id}>
                {index === 5 && (
                  <motion.div
                    ...
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.6, delay: 5 * 0.1, ease }}
                    className={`relative pl-12 md:pl-0 mb-12 ${
                      5 % 2 === 0 ? 'md:pr-[52%]' : 'md:pl-[52%]'
                    }`}
                  >
                ...
                <motion.div
                  ...
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, delay: visualIndex * 0.1, ease }}
                  className={`relative pl-12 md:pl-0 mb-12 ${
                    visualIndex % 2 === 0 ? 'md:pr-[52%]' : 'md:pl-[52%]'
                  }`}
                >
                  {/* Timeline dot */}
                  <div className="absolute left-2 md:left-1/2 md:-translate-x-1/2 top-6 w-4 h-4 rounded-full bg-gradient-accent border-4 border-white z-10" />
